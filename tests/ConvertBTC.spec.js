...
describe('ConvertBTC', () => {
  ...
  it('should use currency USD and 1 as amount default', (done) => {
    ...
    convertBTC();
    setTimeout(() => {
      expect(consoleStub).to.have.been.calledWith(`${chalk.red(1)} BTC to ${chalk.cyan('USD')} = ${chalk.yellow(4654.13)}`);
      done();
    }, 300)
  });
  ...
  it('should use currency USD and 10 as amount', (done) => {
    ...
    nock('https://apiv2.bitcoinaverage.com')
     ...
     convertBTC('USD', 10);
     setTimeout(() => {
      expect(consoleStub).to.have.been.calledWith(`${chalk.red(10)} BTC to ${chalk.cyan('USD')} = ${chalk.yellow(4654.13)}`);
      done();
    }, 300)
  });
  ...
  it('should use currency BRL and 1 as amount default', (done) => {
    ...
    nock('https://apiv2.bitcoinaverage.com')
     ...
     convertBTC('BRL');
     setTimeout(() => {
      expect(consoleStub).to.have.been.calledWith(`${chalk.red(1)} BTC to ${chalk.cyan('BRL')} = ${chalk.yellow(4654.13)}`);
      done();
    }, 300)
  });
